        ...
        <div className="row mt-4 mb-4 p-0 ms-1 ms-md-2 align-items-center hover-shadow" style={{ cursor: "pointer" }}>
            ...
            <div className="col-12 col-md-5 row p-0 ms-1 ms-md-2">
                <div className="col-12 col-md-6 mb-2 mb-md-0 d-flex justify-content-center align-items-center">
                    ...
                    >
                        {concluido ? "Transferir Certificado" : `${percentagem}% concluído`}
                    </button>
                </div>
                    ...
                    if (data.success) {
                        ...
                        Promise.all(
                            cursosComFormador.map(curso =>
                                axios.get(`http://localhost:3000/progresso-aula/progresso/${user.id}/${curso.id}`, { headers: authHeader() })
                                    .then(res => ({
                                        cursoId: curso.id,
    ...
    async function handleTransferirCertificado(inscricaoId) {
        try {
            const response = await axios.get(
                `http://localhost:3000/certificado/download/${inscricaoId}`,
                {
                    headers: { ...authHeader() },
                    responseType: "blob"
                }
