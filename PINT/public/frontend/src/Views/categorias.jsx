...
const Categorias = () => {
    ...
    const getFiltroAtivoTexto = useCallback(() => {
    ...
    }, [categoriaSelecionada, areaSelecionada, topicoSelecionado, todasCategorias, todasAreas, todasTopicos]);

        useEffect(() => {
        const filtro = getFiltroAtivoTexto();
        if (filtro && filtro !== "Área de tópicos") {
            document.title = `${filtro} / SoftSkills`;
        } else {
            document.title = "Categorias / SoftSkills";
        }
    }, [getFiltroAtivoTexto]);
