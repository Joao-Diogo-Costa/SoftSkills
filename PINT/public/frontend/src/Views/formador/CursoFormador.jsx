...
function CursoFormador() {
    ...
    useEffect(() => {
        axios.get("https://pint-web-htw2.onrender.com/curso/list-formador", { headers: authHeader() })
    ...
    useEffect(() => {
        ...
        axios.get("https://pint-web-htw2.onrender.com/area/list").then(res => setAreas(res.data.data || []));
        axios.get("https://pint-web-htw2.onrender.com/topico-curso/list").then(res => setTopicos(res.data.data || []));
    }, []);
    ...
    const areasFiltradas = categoriaFiltro
        ? areas.filter(a => String(a.categoriaId) === String(categoriaFiltro))
        : areas;

    const topicosFiltrados = areaFiltro
        ? topicos.filter(t => String(t.areaId) === String(areaFiltro))
        : topicos;

    // Filtrar cursos conforme filtros e pesquisa
    const cursosFiltrados = cursos.filter(curso => {
        const matchCategoria = categoriaFiltro
            ? String(curso.TOPICOC?.AREAC?.categoriaId) === String(categoriaFiltro)
            : true;
        const matchArea = areaFiltro
            ? String(curso.TOPICOC?.AREAC?.id) === String(areaFiltro)
            : true;
        const matchTopico = topicoFiltro
            ? String(curso.TOPICOC?.id) === String(topicoFiltro)
            : true;
        const matchSearch =
            curso.nome.toLowerCase().includes(search.toLowerCase()) ||
            (curso.descricaoCurso || "").toLowerCase().includes(search.toLowerCase());
                                ...
                                {showFiltro && (
                                    <div ref={filtroRef} className="shadow rounded p-3" style={{ position: "absolute", top: "110%", left: 0, background: "#fff", zIndex: 10, minWidth: 220 }}>
                                        <div className="mb-2">
                                            ...
                                            <Select
                                                options={[{ value: "", label: "Todas" }, ...categorias.map(cat => ({ value: String(cat.id), label: cat.nome }))]}
                                                value={
                                                    categoriaFiltro
                                                        ? { value: categoriaFiltro, label: categorias.find(cat => String(cat.id) === String(categoriaFiltro))?.nome }
                                                        : { value: "", label: "Todas" }
                                                }
                                                onChange={option => setCategoriaFiltro(option && option.value !== undefined ? String(option.value) : "")}
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <Select
                                                options={[{ value: "", label: "Todas" }, ...areasFiltradas.map(area => ({ value: String(area.id), label: area.nome }))]}
                                                value={
                                                    areaFiltro
                                                        ? { value: areaFiltro, label: areasFiltradas.find(area => String(area.id) === String(areaFiltro))?.nome }
                                                        : { value: "", label: "Todas" }
                                                }
                                                onChange={option => setAreaFiltro(option && option.value !== undefined ? String(option.value) : "")}
                                                placeholder="Área"
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <Select
                                                options={[{ value: "", label: "Todos" }, ...topicosFiltrados.map(topico => ({ value: String(topico.id), label: topico.nomeTopico }))]}
                                                value={
                                                    topicoFiltro
                                                        ? { value: topicoFiltro, label: topicosFiltrados.find(topico => String(topico.id) === String(topicoFiltro))?.nomeTopico }
                                                        : { value: "", label: "Todos" }
                                                }
                                                onChange={option => setTopicoFiltro(option && option.value !== undefined ? String(option.value) : "")}
                                                placeholder="Tópico"
