...
const Navbar = () => {
    ...
    const [notifications, setNotifications] = useState([]); // Add this with other state declarations
    const [notificationsOpen, setNotificationsOpen] = useState(false);
    const navigate = useNavigate();

    useEffect(() => {
        fetch("http://localhost:3000/categoria/list")
            .then(res => res.json())
            .then(data => {
        ...
        fetch("http://localhost:3000/area/list")
            .then(res => res.json())
            .then(data => {
                if (data.success && Array.isArray(data.data)) {
                    setAreas(data.data);
        ...
        fetch("http://localhost:3000/topico-curso/list")
            .then(res => res.json())
            .then(data => {
                if (data.success && Array.isArray(data.data)) {
