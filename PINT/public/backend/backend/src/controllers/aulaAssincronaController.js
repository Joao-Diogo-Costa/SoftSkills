...
controllers.aulaAssincrona_update = async (req, res) => {
  try {
    ...
    const aula = await AulaAssincrona.findByPk(id);
    if (!aula) {
      return res.status(404).json({ erro: "Aula assíncrona não encontrada." });
    }
    ...
    res.status(500).json({ erro: "Erro ao atualizar aula assíncrona.", details: error.message });
  }
};
...
controllers.aulaAssincrona_delete = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao apagar aula assíncrona.", details: error.message });
  }
};
