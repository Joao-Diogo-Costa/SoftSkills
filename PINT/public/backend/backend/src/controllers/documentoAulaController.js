...
controllers.documentoAula_list = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao listar documentos de aula.", error });
  }
};
...
controllers.documentoAula_detail = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao buscar documento de aula.", error });
  }
};
...
controllers.documentoAula_create = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao criar documento de aula.", error: error.message });
  }
};
...
controllers.documentoAula_update = async (req, res) => {
  try {
    ...
    const documentoAula = await DocumentoAula.findByPk(id);
    if (!documentoAula) {
      return res.status(404).json({ error: "Documento de aula não encontrado." });
    }
    ...
    res.status(500).json({ success: false, message: "Erro ao atualizar documento de aula.", error: error.message });
  }
};
...
controllers.documentoAula_delete = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao apagar documento de aula.", error: error.message });
  }
};
