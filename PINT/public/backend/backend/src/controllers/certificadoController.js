...
controllers.certificado_list = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao listar certificados.", error });
  }
};
...
controllers.certificado_detail = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao buscar certificado.", error });
  }
};
...
controllers.certificado_create = async (req, res) => {
  try {
    ...
    if (!formandoId) {
      return res.status(400).json({ error: "ID do formando é obrigatório." });
    }
    ...
    res.status(500).json({ success: false, message: "Erro ao criar certificado.", error: error.message });
  }
};
...
controllers.certificado_update = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao atualizar certificado.", error: error.message });
  }
};
...
controllers.certificado_delete = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao apagar certificado.", details: error.message });
  }
};
