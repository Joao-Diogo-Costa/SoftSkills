...
controllers.sugestao_create = async (req, res) => {
  try {
    ...
    if (!formando) {
      return res.status(404).json({ message: "Formando não encontrado com esse ID." });
    }
...
controllers.sugestao_update = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erro ao atualizar sugestão.", details: error.message });
  }
};
...
controllers.sugestao_delete = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erro ao apagar sugestão.", details: error.message });
  }
};
