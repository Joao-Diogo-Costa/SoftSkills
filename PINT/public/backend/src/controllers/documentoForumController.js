...
controllers.documento_list = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Erro ao listar documentos.", details: error.message });
  }
};
...
controllers.documento_detail = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Erro ao buscar documento.", details: error.message });
  }
};
...
controllers.documento_create = async (req, res) => {
  try {
    ...
    if (!comentarioId) {
      return res.status(400).json({ message: "Comentário ID é obrigatório." });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Erro ao criar documento.", details: error.message });
  }
};
...
controllers.documento_update = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao atualizar documento.", details: error.message });
  }
};
...
controllers.documento_delete = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao apagar documento.", details: error.message });
  }
};
