...
controllers.aulaAssincrona_list = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao listar aulas assíncronas.",
      error,
    });
  }
};
...
controllers.aulaAssincrona_detail = async (req, res) => {
  try {
    ...
    if (!aula) {
      return res
        .status(404)
        .json({ success: false, message: "Aula assíncrona não encontrada." });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao obter aula assíncrona.",
...
controllers.aulaAssincrona_create = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao criar aula assíncrona.",
      details: error.message,
    });
  }
};
...
controllers.aulaAssincrona_update = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Erro ao atualizar aula assíncrona.",
        details: error.message,
      });
  }
};
...
controllers.aulaAssincrona_delete = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Aula assíncrona apagada com sucesso.",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Erro ao apagar aula assíncrona.",
      details: error.message,
    });
  }
};
...
controllers.aulaAssincrona_listByCurso = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao listar aulas assíncronas do curso.",
      error: error.message,
    });
  }
};
