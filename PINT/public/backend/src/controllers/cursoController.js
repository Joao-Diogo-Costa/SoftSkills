...
controllers.listarCursosPorCategoria = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Erro ao buscar cursos por categoria.", details: error.message });
  }
};
...
controllers.listarCursosPorArea = async (req, res) => {
  try {
    ...
    res.status(500).json({success: false, message: "Erro ao buscar cursos por área.", details: error.message });
  }
};
