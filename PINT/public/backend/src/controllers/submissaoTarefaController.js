...
controllers.submissao_list = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Erro ao listar submissões.", details: error.message });
  }
};
...
controllers.submissao_detail = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Erro ao buscar submissão.", details: error.message });
  }
};
...
controllers.submissao_update = async (req, res) => {
  try {
    ...
    if (idTarefa) {
      const tarefa = await Tarefa.findByPk(idTarefa);
      if (!tarefa) {
        return res.status(404).json({ message: "Tarefa não encontrada com esse ID." });
      }
    }
    ...
    res.status(500).json({ message: "Erro ao atualizar submissão.", details: error.message });
  }
};
...
controllers.submissao_delete = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erro ao apagar submissão.", details: error.message });
  }
};
