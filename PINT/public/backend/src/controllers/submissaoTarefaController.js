...
//Listar por tarefa
controllers.listarPorTarefa = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: "Erro ao buscar submissões.", details: error.message });
    }
};

//atualizar nota
controllers.atualizarNota = async (req, res) => {
  try {
    const { id } = req.params;
    const { nota } = req.body;
    ...
    res.json({ success: true, message: "Nota atualizada com sucesso.", data: submissao });
  } catch (error) {
    res.status(500).json({
      success: false,
...
controllers.submissao_detail = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Erro ao obter submissão.",
      details: error.message,
    });
  }
};
...
controllers.uploadSubmissaoTarefa = async (req, res) => {
  try {
    ...
    if (tarefa.dataLimite && agora > tarefa.dataLimite) {
      return res.status(400).json({ success: false, message: "A data limite para submissão já passou." });
    }
    ...
    if (!allowedMimeTypes.includes(file.mimetype)) {
      return res
        .status(400)
        .json({ success: false, message: "Tipo de ficheiro não permitido." });
    }

    const fileExtension = path.extname(file.originalname);
    const uniqueFileName = `${idTarefa}-${utilizadorId}-${crypto
      .randomBytes(12)
    ...
    res.json({
      ...
      submissao: {
        ...
        url: submissao.url,
        tipo: submissao.tipo,
        idTarefa: submissao.idTarefa,
        utilizadorId: submissao.utilizadorId,
      },
    });
  } catch (error) {
...
controllers.deleteSubmissaoTarefa = async (req, res) => {
  try {
    ...
    if (!submissaoId) {
      return res
        .status(400)
        .json({ success: false, message: "ID da submissão é obrigatório." });
    }
    ...
    if (!isOwner && !isAdminOrGestor) {
      return res.status(403).json({
        success: false,
        message: "Não tem permissão para eliminar esta submissão.",
      });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Submissão eliminada com sucesso." });
  } catch (error) {
