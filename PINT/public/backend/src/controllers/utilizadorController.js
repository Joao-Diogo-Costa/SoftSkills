...
controllers.utilizador_update = async (req, res) => {
  try {
    ...
    if (nomeUtilizador !== undefined)
      dadosAtualizados.nomeUtilizador = nomeUtilizador;
  ...
  try {
    ...
    if (utilizador.role === "admin") {
      return res
        ...
        .json({
          success: false,
          message: "Não é permitido apagar um utilizador admin.",
        });
    }
...
controllers.uploadImagemPerfil = async (req, res) => {
  try {
    ...
    const uniqueFileName = `${id}-${crypto
      .randomBytes(16)
    ...
    if (
      oldImageUrl &&
      oldImageUrl !== DEFAULT_PROFILE_URL &&
      oldImageUrl !== DEFAULT_ADMIN_URL
    ) {
      ...
      if (oldImageKey) {
        try {
          ...
          console.log(
            `Imagem antiga (${oldImageKey}) removida com sucesso do S3.`
          );
        } catch (s3DeleteError) {
          console.error(
            `Erro ao remover imagem antiga (${oldImageKey}) do S3:`,
    ...
    res.json({
      success: true,
      message: "Imagem enviada e associada ao utilizador com sucesso!",
      imageUrl,
    });
  } catch (error) {
...
// atualizar tokenfcm
controllers.updateTokenFCM = async (req, res) => {
  try {
    const { utilizadorId, tokenFCM } = req.body;
    if (!utilizadorId || !tokenFCM) {
      return res
        .status(400)
        .json({
          success: false,
          message: "utilizadorId e tokenFCM são obrigatórios.",
        });
    }
    ...
    utilizador.tokenFCM = tokenFCM;
    await utilizador.save();
    res.json({ success: true, message: "Token FCM atualizado com sucesso." });
  } catch (error) {
    res
      .status(500)
      .json({
